(() => {
  ...
  const notFound = (msg) => {
    root.innerHTML = `
      <div class="kf-film-empty">
        ...
        <div class="btn-row">
          <a class="btn btn-ghost" href="/lookbook.html">Back to lookbook</a>
        </div>
      </div>
    `;
  ...
  if (!handle) {
    notFound(
      "Add a film handle to the URL, e.g. /lookbook/film.html?h=3m-2080-gloss-black-vinyl-wrap-g12"
    );
    return;
  }
  ...
  fetch(`/api/films/${encodeURIComponent(handle)}`)
    ...
    .then((data) => {
      ...
      if (!blocks.length) {
        blocks.push(
          `<div class="kf-film-block"><p class="p">Garage notes for this colour are empty. Add copy in <code>doc/spreadsheets/vinyl-products.csv</code>, then re-run <code>python3 scripts/import-films-d1.py</code> (upsert keeps filled fields).</p></div>`
        );
      }
